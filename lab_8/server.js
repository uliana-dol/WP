...
app.put('/films/:id', (req, res) => {
    ...
    if (updateIndex === -1) return res.status(404).send('Film not found');
...
app.delete('/films/:id', (req, res) => {
    ...
    if (films.length === initialLength) return res.status(404).send('Film not found');
    res.status(204).send();
});
...
const server = app.listen(PORT, () => {
    ...
    console.log(`Check your website at http://localhost:${PORT}`);
});

server.on('error', (e) => {
    if (e.code === 'EADDRINUSE') {
        console.error('ERROR: Port is busy. Something is already running on this port!');
    } else {
